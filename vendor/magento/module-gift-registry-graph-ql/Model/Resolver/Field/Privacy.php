...
/**
 * Resolves the gift registry privacy
 */
class Privacy implements ResolverInterface
{
    /**
     * @var EnumLookup
     */
    ...
    public function resolve(
        ...
        if ($context->getUserId() === (int) $model->getCustomerId()) {
            $privacy = $this->enumLookup->getEnumValueFromField(
                GiftRegistryDataMapper::GIFT_REGISTRY_PRIVACY_SETTINGS_MAP,
                (string) $model->getData('is_public')
            );
            ...
            throw new LocalizedException(__('The "%1" field is only available for owners.', ['privacy_settings']));
        }
